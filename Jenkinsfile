            ...
            steps{
                script{
                    withAWS(credentials: 'aws-auth', region: 'us-east-1'){
                     sh """
                       aws eks update-kubeconfig --region $REGION --name "$PROJECT-${param.deploy_to}"
                       kubectl get node
                       sed -i "s/IMAGE_VERSION/${param.appVersion}/g", values-${param.deploy_to}.yaml
                       helm upgrade --install $COMPONENT -f values-${param.deploy_to}.yaml .
                     """
                    }
